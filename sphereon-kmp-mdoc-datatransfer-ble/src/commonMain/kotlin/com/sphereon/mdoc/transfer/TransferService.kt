    ...
    override suspend fun startQrEngagement(): String {
        ...
        if (ble) {
            ...
            val bleService = BleService(CoroutineScope(CoroutineName("ble-service")), onStatus = { state, message ->
                {
                    when (state) {
                        IBleService.ScanState.Initial -> sendEvent(Initializing(EngagementRole.HOLDER))
                        IBleService.ScanState.Scanning -> sendEvent(Connecting(EngagementRole.HOLDER))
                        IBleService.ScanState.Canceled -> sendEvent(Canceled(EngagementRole.HOLDER, "Scan canceled"))
                        IBleService.ScanState.Finished -> sendEvent(DebugEvent(EngagementRole.HOLDER, message ?: "Scan finished"))
                        IBleService.ScanState.Error -> sendEvent(Error(EngagementRole.HOLDER, message ?: "Scan error"))
                    }
                }
            })
            this.bleService = bleService
//            bleService.advertisements.collect { advertisement -> println(advertisement)}
    ...
    override fun getEngagementRole() = engagementService?.getRole() ?: EngagementRole.HOLDER

    private fun sendEvent(event: TransferEvent) {
