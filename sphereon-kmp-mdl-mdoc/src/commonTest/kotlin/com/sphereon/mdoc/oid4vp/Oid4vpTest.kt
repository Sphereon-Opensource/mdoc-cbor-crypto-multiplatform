...
class Oid4vpTest {
    ...
    fun shouldCreateDeviceResponseFromMdoc() = runTest {
        ...
        val docsAndDescriptors = MdocOid4vpService().matchDocumentsAndDescriptors(applicableDocuments = applicableDocs, presentationDefinition = pd, mdocNonce = mdocNonce)
        assertEquals(1, docsAndDescriptors.size)
        assertNotNull(docsAndDescriptors[0].deviceKeyInfo)

        val deviceResponse = MdocOid4vpService().createDeviceResponse(matchingDocuments = docsAndDescriptors, presentationDefinition = pd, clientId = "https://test.com", responseUri = "https://test.com/response", authorizationRequestNonce = "auth-nonce")
        assertNotNull(deviceResponse)

        /*val devidceResponse = doc.toSingleDocDeviceResponse(pd)
        assertNotNull(deviceResponse)*/
        assertEquals(
